...
// Método para agregar stock (SUMA al stock actual)
Future<bool> agregarStock() async {
  if (formKey.currentState?.validate() ?? false) {
    try {
      ...
      // Obtener el stock actual para calcular el nuevo stock total
      final productoActual = await ProductoDB.obtenerProductoPorId(
        id: idProducto.value,
        conn: Database.conn,
      );
      ...
      final nuevoStockTotal = productoActual.stock + cantidad;
      
      // Usar asignarStock para establecer el nuevo valor total
      final bool exito = await ProductoDB.asignarStock(
        id: idProducto.value,
      ...
      if (exito) {
        // Recargar el producto actualizado desde la base de datos
        final productoDb = await ProductoDB.obtenerProductoPorId(
          id: idProducto.value,
          conn: Database.conn,
        );
        ...
        // Llamar al callback si existe, pasando el stock actual REAL
        if (onStockUpdated != null && producto.value != null) {
          onStockUpdated!(producto.value!.stock);
        }
