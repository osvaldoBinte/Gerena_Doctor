...
class ShoppingCartController extends GetxController {
  ...
  final RxList<ShoppingCartPostEntity> cartItems =
      <ShoppingCartPostEntity>[].obs;
  final Rx<ShoppingCartResponseEntity?> cartResponse =
  ...
  final RxBool invoiceRequired = false.obs;
  ...
  Future<void> loadAvailablePoints() async {
    try {
      ...
      if (doctorController.doctorProfile.value != null) {
        final newPoints =
            doctorController.doctorProfile.value!.puntosDisponibles ?? 0;
        ...
        if (availablePoints.value != newPoints) {
          ...
          if (usePoints.value && pointsToUse.value > availablePoints.value) {
            print(
                '⚠️ Los puntos seleccionados exceden los disponibles, ajustando...');
  ...
  void toggleUsePoints(bool value) async {
    ...
    print(
        '💳 Usar puntos: $value | Puntos a usar: ${pointsToUse.value} | Descuento: ${pointsDiscount.value}');
  }
  ...
  Future<void> updatePointsToUse(int points) async {
    ...
    if (points > maxPointsAvailable) {
      showErrorSnackbar(
          'No tienes suficientes puntos. Máximo disponible: $maxPointsAvailable');
    ...
    try {
      ...
      print(
          '💰 Puntos: $points | Monto orden: \$${totalOrder.toStringAsFixed(2)} | Descuento calculado: \$${calculatedDiscount.toStringAsFixed(2)}');
    } catch (e) {
  ...
  Future<void> _checkAndLoadCart() async {
    final buyNowJson =
        await _prefs.loadPrefs(type: String, key: AppConstants.buynowKey);
  ...
  Future<void> confirmOrder() async {
    ...
    try {
      ...
      print(
          '📦 Creando pedido con usepoints: ${orderEntity.usepoints}, pointstouse: ${orderEntity.pointstouse}, descuento: \$${pointsDiscount.value}');
      ...
      if (usePoints.value) {
        print(
            '💰 Se usaron ${pointsToUse.value} puntos con descuento de \$${pointsDiscount.value.toStringAsFixed(2)} en el pedido');
      }
      ...
      showErrorSnackbar(
          'No se pudo crear el pedido: ${cleanExceptionMessage(e)}');
    } finally {
  ...
  Future<void> loadCartFromPreferences() async {
    try {
      final cartJson =
          await _prefs.loadPrefs(type: String, key: AppConstants.cartKey);
  ...
  Future<void> saveCartToPreferences() async {
    try {
      ...
      _prefs.savePrefs(
          type: String, key: AppConstants.cartKey, value: cartJson);
  ...
  Future<void> addToCart({
    ...
    try {
      ...
      final existingIndex =
          cartItems.indexWhere((item) => item.medicamentoId == medicamentoId);
  ...
  Future<void> _saveBuyNowToPreferences() async {
    try {
      ...
      _prefs.savePrefs(
          type: String, key: AppConstants.buynowKey, value: itemJson);
  ...
  Future<void> _sendWhatsAppToVendor({
    ...
    try {
      ...
      if (usePoints.value && pointsToUse.value > 0) {
        mensaje +=
            '🎁 *Puntos usados:* ${pointsToUse.value} pts (-\$${pointsDiscount.value.toStringAsFixed(2)} MXN)\n\n';
      }
      ...
      if (response != null) {
        for (var item in response.itenms) {
          mensaje +=
              '• ${item.nombreMedicamento} (x${item.cantidadSolicitada}) - \$${item.precioActual.toStringAsFixed(2)} MXN\n';
        }
      }
